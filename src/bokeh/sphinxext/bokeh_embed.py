...
# -----------------------------------------------------------------------------
"""Compile ``bokeh-embed`` directives into page-level embedding artifacts.
...
_RENAMED_CONFIG_VALUES = {
    "bokeh_plot_resources": "bokeh_embed_resources",
    "bokeh_plot_resource_options": "bokeh_embed_resource_options",
    "bokeh_plot_callback_policy": "bokeh_embed_callback_policy",
}


class _EmbedTiming(NamedTuple):
    total: float
    evaluate: float
    serialize: float
    write: float
...
class BokehEmbedDirective(BokehDirective):
    ...
    def run(self) -> list[Any]:
        ...
        if getenv("BOKEH_SPHINX_QUICK") == "1":
            fallback = nodes.paragraph(
                "", self.options.get("alt", "Interactive Bokeh content omitted from the quick documentation build."),
                classes=["bokeh-embed-fallback"],
            )
        ...
        try:
            ...
            raise SphinxError(f"{location}: bokeh-embed compilation failed: {error}") from error

        record["fallback"] = self.options.get(
            "alt", f"Interactive Bokeh content with {record['root_count']} root(s); view the HTML documentation.",
        )
        page_records = env.bokeh_embed_pages.setdefault(env.docname, [])
        page_index = len(page_records)
        record["page_index"] = page_index
        page_records.append(record)

        dashed_docname = env.docname.replace("/", "-")
        target_id = f"bokeh-embed-{dashed_docname}-{page_index}-{source_fingerprint[:12]}"
        ...
        intro = self.parse(docstring, "<bokeh-embed-docstring>") if docstring and process_docstring else []
    ...
    def process_args_or_content(self) -> tuple[str, str, bool]:
        if self.arguments and self.content:
            raise SphinxError("bokeh-embed:: directive can't have both args and content")

        env = cast(Any, self.env)
        if self.content:
            log.debug(f"[bokeh-embed] handling inline content in {env.docname!r}")
            return "\n".join(self.content), env.bokeh_embed_auxdir, False
        if not self.arguments:
            raise SphinxError("bokeh-embed:: directive requires a source path or inline Python content")

        path = self.arguments[0]
        log.debug(f"[bokeh-embed] handling external content in {env.docname!r}: {path}")
        ...
        try:
            ...
            raise SphinxError(f"bokeh-embed:: error reading {path!r} for {env.docname!r}: {error!r}") from error

    def process_source(self, source: str, path: str, source_fingerprint: str) -> dict[str, Any]:
        env = cast(Any, self.env)
        started = perf_counter()
        cache_file = Path(env.bokeh_embed_cachedir) / f"{source_fingerprint}.json"
        cached = _read_cache(cache_file)
        if cached is not None:
            finished = perf_counter()
            env.bokeh_embed_timings.append(_EmbedTiming(
        ...
        if callback_policy not in ("warn", "error", "suppress"):
            raise ValueError("bokeh_embed_callback_policy must be 'warn', 'error', or 'suppress'")
        ...
        for index, output in enumerate(outputs):
            if callable(output) or getattr(output, "_is_a_bokeh_application_class", False):
                raise RuntimeError(
                    f"captured output {index} is a server application; bokeh-embed builds static artifacts only. "
...
class _RemovedBokehPlotDirective(BokehEmbedDirective):

    def run(self) -> list[Any]:
        source_name, line = self.get_source_info()
        location = f"{source_name}:{line}" if source_name is not None else cast(Any, self.env).docname
        raise SphinxError(
            f"{location}: bokeh-plot was renamed to bokeh-embed in Bokeh 4.0; "
            "replace '.. bokeh-plot::' with '.. bokeh-embed::'",
        )


def config_inited(app: Any, config: Any) -> None:
    raw_config = getattr(config, "_raw_config", {})
    for previous, replacement in _RENAMED_CONFIG_VALUES.items():
        if previous in raw_config:
            raise SphinxError(f"{previous} was renamed to {replacement} in Bokeh 4.0")


def builder_inited(app: Any) -> None:
    app.env.bokeh_embed_auxdir = join(app.env.doctreedir, GENERATED_DIR)
    app.env.bokeh_embed_cachedir = join(app.env.bokeh_embed_auxdir, "cache")
    ensuredir(app.env.bokeh_embed_cachedir)
    if not hasattr(app.env, "bokeh_embed_pages"):
        app.env.bokeh_embed_pages = {}
    app.env.bokeh_embed_timings = []
...
def process_artifact_nodes(app: Any, doctree: Any, fromdocname: str) -> None:
    ...
    if app.builder.format != "html":
        for node in artifact_nodes:
            fallback = nodes.paragraph("", node["record"]["fallback"], classes=["bokeh-embed-fallback"])
    ...
    try:
        ...
        raise SphinxError(
            f"{fromdocname}: bokeh-embed resource policy {policy.mode!r} cannot satisfy page requirements: {error}",
        ) from error
...
def purge_doc(app: Any, env: Any, docname: str) -> None:
    if hasattr(env, "bokeh_embed_pages"):
        env.bokeh_embed_pages.pop(docname, None)


def env_merge_info(app: Any, env: Any, docnames: list[str], other: Any) -> None:
    if not hasattr(env, "bokeh_embed_pages"):
        env.bokeh_embed_pages = {}
    docnames_set = set(docnames)
    for docname in docnames_set:
        records = getattr(other, "bokeh_embed_pages", {}).get(docname)
        if records is not None:
            env.bokeh_embed_pages[docname] = records
    env.bokeh_embed_timings.extend(item for item in other.bokeh_embed_timings if item.docname in docnames_set)
...
def build_finished(app: Any, exception: Exception | None) -> None:
    ...
    pages = getattr(app.env, "bokeh_embed_pages", {})
    ...
    _remove_legacy_generated_assets(Path(app.builder.outdir) / "_static" / LEGACY_GENERATED_DIR)

    timings = app.env.bokeh_embed_timings
    if timings:
        ...
        log.info(
            f"Bokeh embed timings: directives={len(timings)} total={total_seconds:.3f}s "
        ...
        for timing in nlargest(5, timings, key=lambda timing: timing.total):
            log.info(
                f"Bokeh embed slow: total={timing.total:.3f}s evaluate={timing.evaluate:.3f}s "
...
def _page_payload_name(docname: str, records: list[dict[str, Any]], policy_identity: Mapping[str, Any]) -> str:
    ...
    return f"bokeh-embed-{_sha256(payload)}.json"


def _resource_policy(app: Any, docname: str) -> tuple[ResourcePolicy, dict[str, Any]]:
    configured = app.config.bokeh_embed_resources
    options = dict(app.config.bokeh_embed_resource_options or {})
    ...
    if configured is None:
        ...
        raise SphinxError(
            "bokeh_embed_resources must be None, a policy mode, 'static', Resources, ResourcePolicy, or mapping",
        )
    identity = policy.to_dict()
    identity.pop("root_dir", None)
    identity.pop("base_dir", None)
...
def _copy_static_resources(app: Any, pages: Mapping[str, list[dict[str, Any]]]) -> set[str]:
    static_pages = [docname for docname in pages if (
        app.config.bokeh_embed_resources == "static" or (
            isinstance(app.config.bokeh_embed_resources, Mapping) and
            app.config.bokeh_embed_resources.get("mode") == "static"
        )
    )]
    if not static_pages:
        return set()
...
def _is_tracked_asset(name: str, *, payload_prefix: str = GENERATED_DIR) -> bool:
    if name == BOOTSTRAP_NAME or re.fullmatch(rf"{re.escape(payload_prefix)}-[0-9a-f]{{64}}\.json", name):
        return True
    return re.fullmatch(r"vendor/(?:js|css)/bokeh(?:-[a-z-]+)?(?:\.min)?\.(?:js|css)", name) is not None


def _remove_legacy_generated_assets(generated_dir: Path) -> None:
    tracking = generated_dir / TRACKING_NAME
    previous = _read_json(tracking)
    if not isinstance(previous, dict) or previous.get("schema") != "bokeh.sphinx-assets/v1":
        return
    for name in previous.get("files", []):
        if isinstance(name, str) and _is_tracked_asset(name, payload_prefix=LEGACY_GENERATED_DIR):
            (generated_dir / name).unlink(missing_ok=True)
    tracking.unlink(missing_ok=True)
    for directory in (generated_dir / "vendor" / "js", generated_dir / "vendor" / "css",
            generated_dir / "vendor", generated_dir):
        try:
            directory.rmdir()
        except OSError:
            pass
...
def _read_cache(path: Path) -> dict[str, Any] | None:
    ...
    try:
        ...
        log.warning(f"ignoring invalid bokeh-embed cache entry {path}")
        return None
    return value
