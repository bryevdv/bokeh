...
type QuadLikeView = SingleMarkerGlyphView & {
...
export class QuadGL extends LRTBGL {
  protected _data_origin: Vec2 = [0, 0]
  protected _data_error: Vec2 = [0, 0]

  constructor(regl_wrapper: ReglWrapper, override readonly glyph: QuadLikeView) {
  ...
  override maps_coordinate(attr: string): boolean {
    return attr == "left" || attr == "right" || attr == "top" || attr == "bottom"
  }
  ...
  private _write_item(
    ...
    const {left, top, right, bottom} = this.glyph
    const tx0 = transform_data_value(left[i], mapping.x.kind)
    const ty0 = transform_data_value(top[i], mapping.y.kind)
    const tx1 = transform_data_value(right[i], mapping.x.kind)
    const ty1 = transform_data_value(bottom[i], mapping.y.kind)
  ...
  private _stream_data_geometry(data_mapping: DataMapping): boolean {
    ...
    for (let i = start; i < nmarkers; i++) {
      ...
      const [x_error, y_error] = this._write_item(
        i, data_mapping, centers, second_x, second_y, this._data_origin, physical_i,
      )
  ...
  protected override _set_data(): void {
    ...
    const {origin, error} = pack_data_rects(
      centers, second_x, second_y,
      this.glyph.left, this.glyph.top, this.glyph.right, this.glyph.bottom,
      data_mapping,
    )
