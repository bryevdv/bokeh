...
def _wait_for_document(target: Any) -> None:
    target.wait_for_function(
        "() => document.querySelector('[data-bokeh-root]')?.bokehMount?.state === 'ready'",
        timeout=30_000,
    )
...
def _set_fourier_terms(target: Any, terms: int) -> None:
    target.evaluate(
        "terms => { document.querySelector('[data-bokeh-root]').bokehMount.document"
        ...
        try:
            ...
            for page in pages:
                ...
                initial.append(page.evaluate(
                    "document.querySelector('[data-bokeh-root]').bokehMount.document"
                    ".get_model_by_name('shared-phase').text",
                ))

            for page, text in zip(pages, initial):
                page.wait_for_function(
                    "text => document.querySelector('[data-bokeh-root]').bokehMount.document"
    ...
    with _running_app("streamlit_particles.app:app") as url:
        ...
        try:
            ...
            particle_x = frames[0].evaluate(
                "Number(document.querySelector('[data-bokeh-root]').bokehMount.document"
                ".get_model_by_name('particles').data.x[12345])",
            )
            frames[0].wait_for_function(
                "x => Number(document.querySelector('[data-bokeh-root]').bokehMount.document"
