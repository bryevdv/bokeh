...
function random_int(random: LCGRandom, stop: number): number {
  return Math.floor(random.float()*stop)
}
...
describe("WebGL seeded state machines", () => {
  it.no_image("should match the Canvas hit-test oracle through mixed interaction and mutation traces", async () => {
    const random = new LCGRandom(0x5eed)
    const n = 240
    ...
    const source = new ColumnDataSource({data: {
      ...
      size: range(n).map(() => 6 + random_int(random, 8)),
      marker: range(n).map(() => marker_types[random_int(random, marker_types.length)]),
      color: range(n).map(() => `hsl(${random_int(random, 360)} 70% 45%)`),
    }})
    const attrs = {
    ...
    for (let step = 0; step < 2; step++) {
      for (let mutation = 0; mutation < 8; mutation++) {
        const index = random_int(random, n)
        source.patch({
          x: [[index, 8*random.float() - 4]],
          y: [[index, 8*random.float() - 4]],
          size: [[index, 5 + random_int(random, 12)]],
          marker: [[index, marker_types[random_int(random, marker_types.length)]]],
        })
      }
      source.selected.indices = range(6).map(() => random_int(random, n))
  ...
  it.no_image("should preserve randomized polygon topology across range and data state changes", async () => {
    const random = new LCGRandom(0xc0ffee)
    ...
    for (let generation = 0; generation < 3; generation++) {
      ...
      for (let i = 0; i < npolygons; i++) {
        const count = 3 + random_int(random, 8)
