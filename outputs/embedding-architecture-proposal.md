...
5. Existing APIs remain as compatibility facades and delegate to that pipeline.
...
- Allow a gradual migration with thin compatibility wrappers and no flag day.
...
- Preserving byte-for-byte HTML or JavaScript generated by legacy APIs. Semantic compatibility is the target.
...
| Fetch standalone plot data from a web endpoint | `json_item()` plus `Bokeh.embed.embed_item()` | Single model only; target is embedded in data; no dependency metadata; separate schema | `embed(plot).to_json()` plus `mount(artifact, {targets})`; legacy schema adapter remains |
| Host plot data externally and place one self-loading tag in HTML | `autoload_static()` | Returns JavaScript to save plus a tag; all bundles; race-prone global loader; multi-root broken | Declarative external artifact reference plus one shared bootstrap; legacy wrapper can still emit old JS/tag |
...
| Mount raw standalone docs and root mappings in JavaScript | `Bokeh.embed.embed_items()` | Exposes Python's internal `RenderItem` wire shape; mixes standalone and server cases | Accept the versioned artifact schema; keep `embed_items()` as a legacy decoder |
| Mount a single legacy JSON item | `Bokeh.embed.embed_item()` | Repackages its input into random `docid` plus `RenderItem`; target typing disagrees with Python `None` | Legacy adapter to `mount()` |
...
These should be fixed in legacy APIs as small independent bugs, but a shared compiler prevents the same drift from recurring.
...
## Proposed public API and legacy mapping
...
| Legacy API | New equivalent | Compatibility plan |
|---|---|---|
| `file_html(obj, resources, title, ...)` | `embed(obj).page(resources=..., title=..., ...)` | Delegate immediately; preserve signature and template context during migration |
| `components(obj)` | `embed(obj).fragment(resources="none")` | Keep tuple return as a facade; add typed fragment to docs as preferred API |
| `components(..., wrap_script=False, wrap_plot_info=False)` | Typed `Fragment` fields and logical roots | Deprecate wrapping booleans after typed result is stable; they encode return types in flags |
| `json_item(obj, target=...)` | `embed(obj).to_json()` plus mount-time target | Continue emitting/accepting legacy `JsonItem`; add multi-root artifact JSON as preferred form |
| `autoload_static(obj, resources, script_path)` | Save/serve `artifact.to_json()` and emit `artifact.external(payload_url=...)` | Deprecate after docs and examples migrate; legacy JS/tag generator remains for a full major cycle |
| `server_document(url, ...)` | `embed_server(url, ...).fragment()` | Delegate once JSON server bootstrap exists; preserve `/autoload.js` endpoint |
| `server_session(model, session_id, url, ...)` | `embed_server(url, session_id=..., roots=...)` | Warn on `model` once explicit root selection works; document that old releases ignored it |
| `notebook_content()` | notebook renderer for the same artifact | Remain internal, substantially simplified |
| `Bokeh.embed.embed_item()` | `Bokeh.embed.mount(legacyAdapter(item), targets)` | Keep adapter for at least two major versions |
| `Bokeh.embed.embed_items()` | `mount()` for a versioned artifact | Mark raw `RenderItem` input legacy after cross-language artifact support ships |
| `add_document_standalone()` | Internal primitive used by `mount()` | May stay available for expert BokehJS use, but return/participate in `EmbedHandle` lifecycle |
| `add_document_from_session()` | Internal server-source primitive used by `mount()` | Same lifecycle and error contract; document only if it remains intentionally public |

### Migration principles

- Introduce new APIs without deprecation warnings first.
- Convert existing APIs to the shared compiler behind the scenes before asking users to migrate.
- Preserve input conveniences: single object, sequence, ordered mapping, `Document`, current `Resources` values, themes, templates, headers, credentials, and reverse-proxy options.
- Preserve the legacy JSON decoder in BokehJS even after Python prefers the new schema.
- Test semantic equivalence, not generated whitespace or random IDs.
- Publish a cookbook with one before/after recipe for every row in the use-case table, including Flask/Django/Jinja, fetch/JSON, React/Vue, notebook handles, server sessions, and static docs.
- Give custom extension authors a validation command for the new dependency metadata and retain legacy inference as a warning-backed fallback.
- Do not remove `components()` or `file_html()` merely because a new object API exists. They are useful conveniences. The goal is that they stop being independent implementations.
...
- legacy adapter behavior for every current function.
...
### 8. Legacy migration tests

Keep runnable fixtures for the documented integrations:

- `components()` in Jinja/Flask/Django-style templates;
- `json_item()` fetched from an endpoint;
- `autoload_static()` with one and multiple embeds;
- `file_html()` with default, source-string, `Template`, and `Path` templates;
- `save()`/`show()` output state;
- `server_document()` new sessions;
- `server_session()` existing sessions;
- old `Bokeh.embed.embed_item()` and `embed_items()` payloads.

Compare behavior, root placement, resources, theme, callbacks, and lifecycle. Avoid brittle whole-output snapshots except for deliberately versioned schema fixtures.
...
- Add a compatibility conversion from an artifact to current `docs_json`/`RenderItem` so existing renderers can adopt the compiler incrementally.
...
src/bokeh/embed/
  ...
  legacy.py         # existing public API adapters
```
...
- Implement schema validation and legacy adapters.
...
### Phase 3: New Python surface and legacy delegation

- Expose the new artifact constructors/renderers as provisional, then stable after one release of feedback.
- Route `file_html()` and `components()` through them first.
- Route `json_item()` and notebook standalone output next.
- Keep legacy return shapes and schemas at the boundaries.
...
This phase should happen before deprecating autoload because it is the largest in-tree user and the best stress test.
...
- Retain `/autoload.js` as a compatibility endpoint.

### Phase 6: Documentation, deprecations, and cleanup

- Publish the complete migration cookbook.
- Deprecate `autoload_static()` and return-shape flags such as `wrap_plot_info` only after replacements are stable.
- Decide separately whether convenience names such as `components()` and `file_html()` should remain permanently as thin facades.
- Mark raw `RenderItem` browser input and private loader globals unsupported.
- Remove legacy implementation layers only in a major release and only after telemetry/issues show that adapters are no longer carrying important use cases.
...
Legacy Jinja variables and macros should be populated by the page renderer during migration. The new typed result should distinguish a `Path` from template source text and validate missing required blocks/targets early. Do not infer “filename or source” from an arbitrary string.
...
Python legacy functions and the legacy `JsonItem` decoder should have a longer support window than private templates and `RenderItem` internals. A two-major browser decoder window is a reasonable minimum because JSON payload producers and BokehJS hosts are often deployed separately.
...
- Existing public APIs delegate to the shared compiler without known semantic regressions.
